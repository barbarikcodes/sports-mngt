...
FOR EACH ROW insert into log_users values(old.userid,'deleted',now());

-- Trigger for insertion
CREATE TRIGGER `insertlog` AFTER INSERT ON `users`
FOR EACH ROW INSERT INTO log_users VALUES(NEW.userid,'inserted',now());

-- Trigger for updation
CREATE TRIGGER `updatelog` AFTER UPDATE ON `users`
FOR EACH ROW insert INTO log_users values(new.userid,'updated',now());
